...
import { ITableResponse, IDataResponse } from 'gachain/api';
...
export const moveTreeTagEpic: Epic = (action$, store) => action$.ofAction(actions.moveTreeTag)
    .flatMap(action => {
        const state = store.getState();
...
export const getPageTreeDoneEpic: Epic = (action$, store) => action$.ofAction(actions.getPageTree.done)
    .map(action =>
        actions.saveConstructorHistory({ pageID: action.payload.params.id })
    );
